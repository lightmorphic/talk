# Lightmorphic Talk — security and code audit
...
Lightmorphic Talk runs as you, on your machine, with no server, no accounts and no
...
**A cleanup watcher could be aimed at a temporary path.** Lightmorphic Talk leaves a
...
**The update is not signed.** Lightmorphic Talk trusts TLS and the fixed GitHub
...
can synthesise keystrokes; that is why Lightmorphic Talk needs no permission there,
and equally why any other program on that session could watch or fake
input. This is a property of X11, not of Lightmorphic Talk. Wayland's portal exists
precisely to end it, and on Wayland Lightmorphic Talk asks.
