...
- **Restart Lightmorphic Talk**: Settings → Maintenance → Restart Lightmorphic Talk.
...
- **Update**: the dot next to the version number shows the state.
  ...
  click it and Lightmorphic Talk updates and restarts itself. Red means the check
...
## If Lightmorphic Talk won't start

1. Reboot once (fixes most things).
2. Still stuck? Open a terminal and run:
   ```bash
   ~/talk/scripts/talk.sh
   ```
   The error it prints says what's wrong. `data/talk.log` has detail.

## Roll back a bad update

```bash
cd ~/talk && git checkout "tags/$(cat data/previous-version.txt)" && ./scripts/talk.sh
```
...
Unzip it, then copy the `data` folder over `~/talk/data` and restart
Lightmorphic Talk from the tray (or run the launcher above).

## Start fresh

Delete `~/talk/data` and restart. Settings, dictionary and history
